...
\copy healthmap_country (id, name, centroid_override) FROM 'healthmap_country.txt' (ENCODING utf8, NULL '')
...
\copy disease_group (id, parent_id, name, group_type, public_name, short_name, abbreviation, is_global, validator_disease_group_id, weighting, min_new_locations_trigger, min_data_volume, min_distinct_countries, high_frequency_threshold, min_high_frequency_countries, occurs_in_africa, use_machine_learning) FROM 'disease_group.txt' (ENCODING utf8, NULL '')
\copy disease_extent (disease_group_id, min_validation_weighting, max_months_ago_for_higher_occurrence_score, lower_occurrence_score, higher_occurrence_score) FROM 'disease_extent.txt' (ENCODING utf8, NULL '')
...
-- \copy (select id, name, centroid_override from healthmap_country order by id) TO 'healthmap_country.txt' (ENCODING utf8, NULL '')
...
-- \copy (select id, parent_id, name, group_type, public_name, short_name, abbreviation, is_global, validator_disease_group_id, weighting, min_new_locations_trigger, min_data_volume, min_distinct_countries, high_frequency_threshold, min_high_frequency_countries, occurs_in_africa, use_machine_learning from disease_group order by id) TO 'disease_group.txt' (ENCODING utf8, NULL '')
-- \copy (select disease_group_id, min_validation_weighting, max_months_ago_for_higher_occurrence_score, lower_occurrence_score, higher_occurrence_score from disease_extent order by disease_group_id) TO 'disease_extent.txt' (ENCODING utf8, NULL '')

...
UPDATE location SET resolution_weighting=((1.0/ln(25.0/115000.0))*ln((1.0/115000.0)*(SELECT area FROM country WHERE country.gaul_code=location.country_gaul_code))) WHERE precision='COUNTRY' AND has_passed_qc=TRUE AND country_gaul_code IS NOT NULL;
UPDATE location SET resolution_weighting=0 WHERE resolution_weighting<0 AND has_passed_qc=TRUE;

...
-- \copy (select id, disease_group_id, location_id, alert_id, occurrence_date, is_validated, machine_weighting, validation_weighting, final_weighting, final_weighting_excl_spatial from disease_occurrence order by id) TO 'disease_occurrence.txt' (ENCODING utf8, NULL '')
-- \copy (select disease_group_id, geom, min_validation_weighting, min_occurrences_for_presence, min_occurrences_for_possible_presence, max_months_ago_for_higher_occurrence_score, lower_occurrence_score, higher_occurrence_score from disease_extent order by disease_group_id) TO 'disease_extent.txt' (ENCODING utf8, NULL '')

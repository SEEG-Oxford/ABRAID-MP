...
\copy location (id, name, geom, precision, geoname_id, healthmap_country_id, admin_unit_qc_gaul_code, admin_unit_global_gaul_code, admin_unit_tropical_gaul_code, has_passed_qc) FROM 'location.txt' (ENCODING utf8, NULL '')
\copy alert (id, feed_id, title, publication_date, url, summary, healthmap_alert_id) FROM 'alert.txt' (ENCODING utf8, NULL '')
\copy disease_occurrence (id, disease_group_id, location_id, alert_id, occurrence_date, is_validated, system_weighting, validation_weighting) FROM 'disease_occurrence.txt' (ENCODING utf8, NULL '')
...
-- \copy (select id, name, geom, precision, geoname_id, healthmap_country_id, admin_unit_qc_gaul_code, admin_unit_global_gaul_code, admin_unit_tropical_gaul_code, has_passed_qc from location order by id) TO 'location.txt' (ENCODING utf8, NULL '')
...
-- \copy (select id, disease_group_id, location_id, alert_id, occurrence_date, is_validated, system_weighting, validation_weighting from disease_occurrence order by id) TO 'disease_occurrence.txt' (ENCODING utf8, NULL '')
